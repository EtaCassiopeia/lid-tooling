# Arrow: redis-store

Redis-backed `Store` implementation. Target for production deployments once the Redis cluster is provisioned (Q3 2026).

## Status

**UNMAPPED** — pending infrastructure. LLD and EARS drafted; no code started.

## References

### HLD
- `docs/high-level-design.md` § Approach / storage

### LLD
- `docs/intent/storage/redis-store-design.md`

### EARS
- `docs/intent/storage/redis-store-specs.md` (4 specs, prefix `USH-RDIS-*`)

### Tests
- (none yet)

### Code
- (none yet)

## Spec Coverage

| Category | Spec range | Implemented | Active gap | Deferred |
|---|---|---|---|---|
| All USH-RDIS | USH-RDIS-001..004 | 0 | 4 | 0 |
